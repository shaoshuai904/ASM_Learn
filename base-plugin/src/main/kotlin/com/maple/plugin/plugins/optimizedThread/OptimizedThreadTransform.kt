...
import com.maple.plugin.utils.insertArgument
import com.maple.plugin.utils.nameWithDesc
import com.maple.plugin.utils.simpleClassName
 ...
 */
class OptimizedThreadTransform(
    project: Project,
                        ...
                        when (instruction.opcode) {
                            Opcodes.INVOKESTATIC -> {
                                ...
                                if (methodInsnNode?.owner == executorsClass) {
                                    taskList.add {
                                        transformInvokeStatic(classNode, methodNode, instruction)
                                    }
                                }
                            }
                            Opcodes.NEW -> {
                                ...
                                if (typeInsnNode?.desc == threadClass) {
                                    ...
                                    if (!classNode.isThreadFactoryMethod(methodNode)) {
                                        taskList.add {
                                            transformNew(classNode, methodNode, instruction)
                                        }
                                    }
                                }
                            }
